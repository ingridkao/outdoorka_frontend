		...
		if (data?._id) {
			organizer.getActivityParticipant(data._id).then((res: any) => {
				if (res.data) {
					...
					const newParticipantList = res.data.filter((item: any) => item.ticketStatus === 1)
	...
	return (
		<Dialog onClose={() => onClose()} open={open} fullWidth maxWidth="sm">
			...
			<DialogContent sx={{ textAlign: "center" }}>
				...
				<Typography variant="body1">
					已使用：{participantList.length || 0}人 | 報名者：{participantTotal || 0}人
				</Typography>
				<Box>
					<Grid 
						container 
						spacing={1.5} 
						justifyContent={participantList.length === 0?"center":"flex-start"}
					>
